...
describe('Tasks Test Suite', () => {
  beforeAll(async () => {
    ...
    project = await factory.create('Project', { user_id: user._id }) as IProjectTest;
  ...
  it('1 - Should create Tasks on database', async () => {
    await Promise.all(
      tasks.map(async (task: any) => {
        const projectTask = new Task({ title: task.title, project_id: project._id, user_id: user._id });
