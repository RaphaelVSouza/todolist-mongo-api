...
export default async function storeProject(req: Request, res: Response): Promise<Response> {
  const { userId } = req.user  as ISessionUser;

  let { title, description, tasks }: IProject = req.body;
    ...
    await Promise.all(
      tasks.map(async (task: ITask) => {
        if (task.hasOwnProperty('title')) {
          ...
          const projectTask = new Task({ title: task.title, project_id: project._id, user_id: userId });
