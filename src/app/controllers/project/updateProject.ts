...
export default async function updateProject(req: Request, res: Response): Promise<Response> {
    ...
    if (tasks) {
      ...
      await Promise.all(
        tasks.map(async (task: ITask) => {
          const projectTask = new Task({ ...task, project_id: project._id, user_id: userId });
